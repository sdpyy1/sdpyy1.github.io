...
1. 深刻学习RayMarching的性能优化
2. IBL原理捋一遍
3. 弄清楚TAA的解决鬼影的各种细节
4. Vulkan细致过一遍、进一步学习GPU架构
5. GBuffer压缩（https://zhuanlan.zhihu.com/p/479490559里提到的问题）
6. 纹理贴图压缩算法
7. 完成项目的空间加速结构 （八叉树和BVH）https://zhuanlan.zhihu.com/p/696725602
8. 写SSSR 理解RayMarching的各种优化方案
9. ClipMap是什么
10. MVP矩阵捋一遍
11. HIZ遮挡剔除学习
...
> 镜面部分分为两项，一项是光照项 一项是BRDF*cos项
>
> ```
> 最终反射结果 = 环境光本身的信息 × 物体材质的反射特性
> Lo(p,ωo) = [预滤波环境贴图] × [BRDF积分项]
> ```

![image-20260303193126436](image-20260303193126436.png)

首先看光照积分项,为了表达不同的粗糙度，用mipmap存储不同粗糙度下的光照积分结果

在预计算时，记录每个反射方向上的预滤波积分结果



下图意思是就是说对于同一个着色点，我从左图w0看着色点的lobe和从反射方向看向着色点的lobe差距不大。所以我预计算直接预计算右图的情况，实时用的时候把他当作左图去用，也就是采样R方向的。

只要实时计算时的反射方向是R，就可以用预计算方向R去采样

![img](v2-822771782befdee45fea448d133fa209_1440w.jpg)
...
1. 主线程收集完成后，会进入临界区区看状态是否为Idle，如果是则进行交换缓冲区。如果不是则陷入wait。
2. 交换完缓冲区后，执行Kick，让渲染线程开始工作，切换状态为Kick，并执行notify（注意这里的notify）
3. 渲染线程在死循环中等待Kick状态，如果当前是Kick状态，就设置为Busy状态，然后就开始执行它Buffer中存储的各种命令，执行完成后设置自身为Idle状态
