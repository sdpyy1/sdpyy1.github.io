...
> Pool全局只有一个，存储在DynamicRHI中，创建一个RHICommandContext时，用这个Pool来创建一个CommandBuffer来作为这个Context的handle
...
1. 先用极坐标采样完成，再转为3D坐标
2. cos余弦采样的方式就是说我的概率密度函数是正比于cos的，也就是kcossita，然后根据累积积分函数最后要等于1，推出系数应该取多少
...
> 4. 用地差异序列配合余弦重要性采样生成一个采样点的切线空间法线（因为整个随机采样过程，并没有当前的法线参与） 余弦重要性采样的核心是**让采样概率与方向和法向量夹角的余弦值成正比**
...
1. 假的Bindless：Texture2DArray，每个Texture必须有相同的格式
2. 有限的Bindless：在设置资源描述符布局绑定点时，可以设置当前binding 的资源描述符数量不只是1
3. Bindless：开启bindless（创建时设置一个flag）后，binding 的资源描述符数量设置意义变成最大资源描述符数量

> 不开启Bindless时，binding点位是一个数组的情况下，数组每一项指向一个具体的资源，开启bindless后，binding指向的是一张资源表，这就灵活很多了



### PBR

![image-20260314194743012](image-20260314194743012.png)

1. albedo同时表达电解质的反照率和金属的F0，所以在计算F0时，使用![截屏2026-03-14 19.49.23](截屏2026-03-14-19.49.23.png)

来表达金属的F0

### 大气渲染

> 核心思想：1. 太阳光透射衰减后到达某一点 2. 在这一点发生散射 3. 散射到摄像机方向后进一步透射衰减后到达摄像机

![img](v2-121814a840482242b46443719d55a961_1440w-3490153.jpg)

在第二步时，也就是S出，需要先进行散射系数的衰减，他表达的是透射时不按照原方向散射的比例

![img](v2-951d9ef2f4dceb06b3ccfd10f105f6b0_1440w-3490290.jpg)

还需要进一步计算相函数，它表达的意思是即使没有直线透射，还需要计算具体散射到摄像机方向上的比例

![img](v2-f1a78dc668e25d92d2587805f85b6206_1440w-3490365.jpg)

两个衰减相乘，就得到了在某点发生一次散射时，逃逸到摄像机方向上的比例是多少

![截屏2026-03-14 20.13.36](截屏2026-03-14-20.13.36.png)

大气中有两种散射，瑞利散射和米氏散射，他们的散射系数和相函数是不一样的

![截屏2026-03-14 20.20.40](截屏2026-03-14-20.20.40.png)

![截屏2026-03-14 20.21.34](截屏2026-03-14-20.21.34.png)

两个散射系数*相函数的结果相加即可得到S位置的衰减比例

![img](v2-40a72c925e4ce2a3cfdd4eb98c8d2673_1440w.jpg)

到上一张图就解决了S位置的散射衰减，下面来看光线在大气行进过程中的透射衰减

光线在大气行进过程中会发上上边的散射衰减，还会发生吸收

![截屏2026-03-14 20.25.25](截屏2026-03-14-20.25.25.png)

吸收可以用高度

![img](v2-0c6914840bc686e47606034a9c8b94ac_1440w.jpg)

![截屏2026-03-14 20.26.50](截屏2026-03-14-20.26.50.png)

L是传输距离，但是上边这个公式是均匀介质情况下才行，大气是不均匀的，所以计算时是按照一个步长，假设步长中是均匀的来计算，所以计算时，每次步进算一个指数位置的值，把指数位置的值相加，最终再进行指数运算，也就模拟了整个路径上的透射率

![截屏2026-03-14 20.28.40](截屏2026-03-14-20.28.40.png)

![img](v2-420524aae1cde0fb926f8da507f8ee7b_1440w.jpg)

最终计算流程为

![img](v2-f6bf5cb49bdba9daffa0809e57f6b4de_1440w.jpg)

预计算逻辑：在T2流程中，每次步进后需要的透射度可以通过前一部分的透射度*当前步长内的透射度得到。但是T1不行，每次都是不一样的方向，所以对T1进行与计算

![img](v2-5c760c441993877ecd4ae592f52cbdb0_1440w.jpg)

即预计算每个高度，和每个Y方向上的透射度是多少

![img](v2-4055228b35712abec9c292680f0be070_1440w-3491906.jpg)

大气透视查找表没做，他涉及到的是透过大气去看一个Mesh时要考虑透射

### TAA/FXAA

> TAA说清楚几点
>
> 1. 静态场景下的地差异序列取点、偏移值存储在投影矩阵第三行前两个位置
> 2. 动态场景下存储上一帧的Model、View，保存速度图
> 3. 速度图采样时要进行clamp或者clip（这里涉及到转移到另外一个色彩空间）（进一步重投影还可以考虑一些其他因素：法线不一致、MeshID不一致，偏移量length）
...
>
> 1. 从创建Pass到声明Pass的转变
> 2. 解耦，我并不需要知道我这个资源从哪来，需要发送给哪个pass

RDG的作用：

1. pass的culling：如果要对GBuffer进行Debug，那Light链路最终都不会显示在屏幕上，直接全部Culling
2. 资源布局转换：比如纹理的只读和读写
3. 资源复用：GBuffer纹理在光照用完后后续就不用了，其他Pass需要相同规格的纹理，直接拿过来用就行，用资源池来实现（或者叫memory Aliasing，因为这两个资源的生命周期不重叠，所以可以使用同一套底层的handle）
4. async compute（异步计算）：一些Pass与当前正在执行的Pass没有资源依赖，并且这些Pass后续也需要用到，就可以把它发送到不同的queue去执行

RDG的使用流程：

1. RDG.build：构建完整的图结构
2. RDG.complie:处理图
3. RDG.execute: 真正去解析每个Pass的资源，去执行每个Pass的逻辑

### 路径追踪

路径追踪流程：

1. 确定一条光线的Origin和Target，Origin是摄像机的世界位置，Target可以先计算当前像素的UV，转NDC（深度用1），再转世界坐标得到（加入jetter来抗锯齿），发射光线
2. 在击中点先进行直接光照计算，再通过GGX或余弦重要性采样采样一个方向，作为继续弹射的方向（相当于进行一次蒙特卡洛积分）
3. 俄罗斯轮盘赌：取一个概率P，再取一个随机数，如果随机数小于P，则继续弹射（弹射时携带/p），否则直接返回
4. throughput更新考虑：pdf的值、轮盘赌的值、BRDF的衰减

### DDGI

DDGI流程：

1. 光追Pass：每格探针均匀地向周围发射光线，记录击中点的直接光照结果（Radiance），最为Irrandiance的蒙特卡洛积分采样的样本（可以进行无限弹射效果：计算击中点光照时，同时进行DDGI间接光采样，相当与拿到了两次弹射的结果，每一帧都进行一次，相当于无限弹射），把每格探针每格光线的Radiance信息存储起来（同时还要记录每个击中点的距离）
2. 混合Pass：探针记录的是探针上任何方向的irrandiance，所以通过第一步的采样结果，去做半球上的蒙特卡洛积分，得到一个方向上的Irrandiance估计值，在球上均匀选择36个方向，通过八面体映射为6*6的纹理，外边在加一圈防止双线性采样到别的探针，同时记录的距离也进行相似的操作，记录均值和平方的均值。
3. 光照Pass：三种插值方式（1）三线性插值（2）方向插值（3）切比雪夫漏光检测插值     注意采样时选择是法线方向，因为我预计算的就是法线方向上的半球Irrandiance的结果
