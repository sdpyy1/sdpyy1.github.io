...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/30420a057d4b48d1ac427d38c5392200.png)

父关节的旋转会让子关节也跟着旋转
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/487d233707f2467eb57598a1f1b105e9.png)
子关节的朝向就是夫关节朝向乘以自身的旋转
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/c5ef766273b74f7798dcde2d2431193f.png)
只旋转R0时,Q1在Q0的局部坐标系下的坐标并不会变一直是l0,如果想求Q1的世界坐标,就是父关节的世界坐标加朝向*l0
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/26526c4801494c039050d004bac0e624.png)
这一套东西就是用传播来更新,比如Q4的局部坐标系的某一个点,可以算出他在Q3的局部坐标系的位置,Q3的位置又能计算出他在Q2的位置,最终得到这个点在Q0的位置,最终再换算到世界坐标系,就得到了Q4局部坐标系下某一点在全局坐标系下的位置
这块比较好理解
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/a3dd2947738740b8a3ed38487aace030.png)
BVH文件,用欧拉角表示旋转
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/68e64229085948d2ac4ae0eae284d160.png)
...
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b4c4695c429a408d894e4cac371cbc93.png)
### 2. 前向运动学计算
...
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/9cd6da17016c4d8bb0a71ba5e8f44d0c.png)
# 逆向运动学
已知运动结果求每一个关节的旋转
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/523c2740ff8a489bbf301e897f189a11.png)
IK问题可以有解、无解、多解
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5e31c5f9ee3d4b41bcbe530f51958de1.png)
## 双关节问题
下图两个虚线圆的交点就是关节1应该去的位置，到达后旋转关节1，肯定能让关节2到达指定位置
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/09191a1412b7413cb8b4b24f031951c3.png)

另外一种思路：
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/9ee2f77744404313b353f6b4e0be5ba9.png)
1. 移动关节1，让0-2的距离= 0到x的距离
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/dff6e1244c5e4a85a1e318edc2404993.png)
再旋转关节0即可
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/e3984243737a40ee8295c55bd1c75fc2.png)
在三维下绕着0x为轴旋转即可
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/46d4bc76f25f46fa9198e4e6408ad15a.png)
## 多关节最优化问题
只考虑末端点到达指定位置的问题，建模成 **通过各个关节的旋转，得到末端点位置的这个函数**
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f42d20f408024c66b807662f62c181c2.png)
求解问题就是：就是找到各关节的旋转使得目标点位置-公式求解的末端点位置=0
这个方程有没有根 ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/a83a39c75e6d44b28505bc38fe51b669.png)
等价的可以转化为一个优化问题，优化问题取极值
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/8d0ddd126ff34be3ad0ba4905bf04f05.png)
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/daaebb3f845246a19d2ff1c00482f0d6.png)
求解问题到这里就变成了函数求极值的问题
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/59f12c68be1f4f8ca5caf06bc19e5b46.png)
### 优化问题下降方法
#### 1. 循环坐标下降法 CCD
认为参数的坐标轴方向为下降方向，不断交换坐标轴。 其实就是每次只更新一个参数（一个旋转角度）
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/7bab171e461a44c3bdc96608ae6d92cc.png)
CCD放在IK来解释就是每次只旋转一个关节
下图就是只旋转3，让3-4朝向正确（也就是寻找了x离目标位置最近的位置）
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b8103f4217204a0eb70cc6f78da8302d.png)

下一步旋转关节2
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/aaf51c0f1d8e4c7a88014c5f9cecb5c1.png)
依此类推
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b31cd73e4a4b4100b70025bdf97d8cf0.png)
之后再回到4关节进行循环
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/8d495bf0c26e49e3b095a8899fe78f78.png)
#### 2.  梯度下降（雅可比转置法）
CCD没有考虑到目标函数的性质，直接找负梯度方向就可以更快的下降
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/a1bb1fda63424be8a95f5ee62979d73b.png)
也就是说每个参数的更新方向都是下降最快的方向![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/e4ebf4f5fe21425f82803dda0393847b.png)
参数的更新公式
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/3c958e0be7a347f098c0f43a1badccab.png)
红色的这一项是控制梯度更新是往正方向还是负方向（因为下降过头还得回来）
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/d5cbec56d7f04707b1036b396aef142f.png)
前边梯度的倒数构成的矩阵也可以叫做雅可比矩阵（f对每个参数求导的结果组成的矩阵）
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/086df6c95162428c814ec88e36cf060a.png)
下降梯度法也可以就雅可比转置法。
对于IK问题，他输出是一个点，是三维的输出（x,y,z）所以雅可比矩阵应该是三维的
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b45b38c520104f5ba5bd8bcd6365baff.png)
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/0d7028d05b824fc08e40b90d524b2c45.png)
##### 雅可比矩阵计算方法
1. 调库！！！
2. 有限差分（用前向运动学运动依次来获得数据填充矩阵，看不懂。。。。）
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/0bdd5647b2a84cc19081b7e3a4e65443.png)
3. 几何方法
（针对单自由度关节）这里应该是当一个关节旋转一个很小的角度时，把罗德里格斯旋转公式求出目标点，取极限求出来的就是雅可比矩阵的一项
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/9ca61d9917d745799f1546d4cd652f48.png)
（针对ball 3自由度的关节）很难理解的，选择不理解，知道是可以的就行
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/346ecc4e5d2545a58626414074e7df36.png)
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b04f38a3641c4be99b353d7ae8b9e9fc.png)
#### 3.  高斯-牛顿法
最优化学过。。忘了，这里选择不回忆
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/6a1ebe08306a44dba0156749759741cc.png)
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/aff67403181642ca985180be00ce10e7.png)
#### 4.  雅可比逆方法
。。。听个响![](https://i-blog.csdnimg.cn/direct/33850882bb744ab6a7088f7923a5b08e.png)
听个。。
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/21260fddf90a4ea385c6696d49e9d117.png)

## 角色的IK
更复杂的优化问题。 每个目标点都对应上一节一个优化目标，最后相加。还需要加一个正则项，约束各个关节的旋转
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/3ac34f45d8f5453385d3d1406f6c5851.png)
## 逆向运动学作业
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/4ab8167d086641ae88099a898c09df2e.png)
## 梯度函数
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/c009bf351cde4c5fb7bb4a1e77f171a2.png)
也可以是离当前点最近的点的值作为f(x)
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f5df7d93340e407fb4ed42fb0e09eebc.png)

## 线性插值函数
直接用直线连接已知点
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/406f498e5b85496cac6c4e98ba6e49dc.png)
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/15117e860a2142ac9245283827ace5cc.png)
### 线性插值的平滑性
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/d51108c9114540598f2f95a026d9388f.png)
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/deb1999fb8824fef9509505dba7418bb.png)
## 多项式插值 Polynomial Interpolation 
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5ddbccd8457e4d459972ecffc786d58a.png)
计算参数就是把已知点带进去
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/d372c045005b4bdbb0bb6d60f171b9c4.png)
然后用矩阵求解。 这里就能看出需要多少个采样点
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/18e43c512d2346d9a17e8ae3bdafa8e7.png)
把矩阵求逆就可以求出参数a
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b7ad98fd79554f44a457f438aac0a76b.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/01cf9890b6474a47a01f866482512ae6.png)
## 样条插值 Spline Interpolation
只在单独几个采样点上用低阶多项式插值
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/fcd34f02bada4671bacb8287547a5cd1.png)
常用的是Cubic Splines （三次样条插值）
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/3fab4c877b7b47a7bbb8057296033dad.png)
计算时只需要相邻的两个采样点计算依次多项式参数。
如果总采样点是N+1，那一共有N段，每段都是一个三次多项式。每个多项式4个未知参数。所以一共有4N个未知参数。
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/4790b9f7979c487f803d56a18b773c99.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/ad7da6c9a3b54b8d8e10b14875a936cc.png)
总结就是 4个方程4个未知数
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/431e38372bf44abaa65f519f88fad3d0.png)
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f243ca6de7ea4264840c08951b771522.png)
矩阵的逆都一步到位了，一次矩阵乘法就直接求解4个位置系数
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/9f814d58223a4ccfb2b3fa816ffdd56c.png)
还可以把多项式直接写出矩阵形式
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/91e86cc0e00d461d9a3ba6f0041721c5.png)
## 旋转的插值
旋转的四种表示方法：
- 旋转矩阵
- 欧拉角
...
下面两种旋转表达旋转速度插值不是恒定的
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/72fbd1c6d1c74e8ca344732c2c35fe8e.png)
