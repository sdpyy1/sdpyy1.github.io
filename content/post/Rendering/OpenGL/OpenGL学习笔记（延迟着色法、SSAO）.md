...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/58a9913f6cf74c1f90dc682485183fd7.png)延迟着色法基于我们延迟(Defer)或推迟(Postpone)大部分计算量非常大的渲染(像是光照)到后期进行处理的想法。
它包含两个处理阶段：
1. 几何处理：渲染场景一次，获取各种几何信息，并存储在一系列叫G缓冲的纹理中。比如位置、颜色、法向量、镜面值
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5d72ce7b8a844d10a11dbfa6f72f7733.png)
2. 光照处理阶段：利用G缓冲的数据来计算光照
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/68d3bc5cf1c44c94af5baa84143a2317.png)
这样做一些被覆盖的物体就不会计算光照
...
```
![请添加图片描述](https://i-blog.csdnimg.cn/direct/96c4315f99c7477096103732f94ee914.png)
...
![请添加图片描述](https://i-blog.csdnimg.cn/direct/65231d4348ac4974aa27b9c8f478cc24.png)
...
![请添加图片描述](https://i-blog.csdnimg.cn/direct/8b57ffc6d79d41dbb5e7faad197f20da.png)
## 更多的光源
...
使用光体积更好的方法是渲染一个实际的球体，并根据光体积的半径缩放。这些球的中心放置在光源的位置，由于它是根据光体积半径缩放的，这个球体正好覆盖了光的可视体积。这就是我们的技巧：我们使用大体相同的延迟片段着色器来渲染球体。因为球体产生了完全匹配于受影响像素的着色器调用，我们只渲染了受影响的像素而跳过其它的像素。下面这幅图展示了这一技巧：![请添加图片描述](https://i-blog.csdnimg.cn/direct/dfe84c918d1c43a695035843f12299e3.png)
# 屏幕空间环境光遮蔽 SSAO
在现实中，光线会以任意方向散射，它的强度是会一直改变的，所以间接被照到的那部分场景也应该有变化的强度，而不是一成不变的环境光。环境光遮蔽（Anbient Occlusion）是其中一种解决方案。

它的原理是通过将褶皱、孔洞和非常靠近的墙面变暗的方法近似模拟出间接光照。这些区域很大程度上是被周围的几何体遮蔽的，光线会很难流失，所以这些地方看起来会更暗一些。![请添加图片描述](https://i-blog.csdnimg.cn/direct/9f4b1a5e36c04e929a340f045b5209ff.png)
...
SSAO背后的原理：每一个片段，我们都会根据周边深度值计算一个遮蔽因子(Occlusion Factor)。这个遮蔽因子之后会被用来减少或者抵消片段的环境光照分量。遮蔽因子是通过采集片段周围球型核心(Kernel)的多个深度样本，并![请添加图片描述](https://i-blog.csdnimg.cn/direct/e24f163150ff437185ff62cb16a048cf.png)
灰色样本就是深度高于片段深度的样本，灰色越多，片段获得的环境光照也就越少。

渲染效果的质量和精度与我们采样样本的数量有直接关系
![请添加图片描述](https://i-blog.csdnimg.cn/direct/56fe26a4f96b486f8d561c25878b6c32.png)
Crytek公司开发的SSAO技术会产生一种特殊的视觉风格。因为使用的采样核心是一个球体，它导致平整的墙面也会显得灰蒙蒙的，因为核心中一半的样本都会在墙这个几何体上。下面这幅图展示了孤岛危机的SSAO，它清晰地展示了这种灰蒙蒙的感觉![请添加图片描述](https://i-blog.csdnimg.cn/direct/3dee13b95c5f4ed88ec374be5062fd2b.png)
由于这个原因，我们将不会使用球体的采样核心，而使用一个沿着表面法向量的半球体采样核心。

![请添加图片描述](https://i-blog.csdnimg.cn/direct/59cccaa7be02426abde6c486cc55f42b.png)
...
![请添加图片描述](https://i-blog.csdnimg.cn/direct/bb62b302bbdc471db788786a32440a01.png)
...
![请添加图片描述](https://i-blog.csdnimg.cn/direct/f92f10fc8f55436a8ccd9748746d83e9.png)
